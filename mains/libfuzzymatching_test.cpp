...
// This example provides a C-Style wrapper over the C++ classes: this might be suitable for
// Java-JNI applications (if not running in a Docker environment: older versions did not allow
// good memory mapping over files while running within the virtual environment, and running in a JVM).
